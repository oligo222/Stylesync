...
def get_real_dashboard_stats():
    """
    Computes real dashboard metrics from actual project data instead of
    hardcoded placeholders. Falls back to safe defaults if files are
    missing or empty (e.g. on a completely fresh run).
    """
    project_root = os.path.dirname(os.path.dirname(__file__))

    # Total Wardrobe: count of real scanned + adapted items
    try:
    ...
    # Outfits Created + Style Match Score: from usage history
    outfits_count = 0
    style_score = 0
    try:
    ...
    # AI Recommendations: from the last pipeline run's output
    requests_pending = 0
    try:
...
def main():
    ...
    # Initialize the user's name in session state (persists across reruns)
    if "user_name" not in st.session_state:
        st.session_state.user_name = "Alex"

    # Simple name input in the sidebar area, above the rest of the page
    with st.sidebar:
    ...
    # Metric cards row — real data, computed from actual project files
