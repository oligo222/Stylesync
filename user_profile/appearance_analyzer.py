    ...
    prompt = """Analyze the person in this profile picture. Identify the following features and return them strictly in JSON format:
{
  ...
  "body_type": "One of: Slim, Athletic, Average, Curvy, Stocky",
...
Do not include any extra markdown formatting like ```json ... ``` or text explanations. Return ONLY the raw JSON string."""
    ...
    try:
        ...
        valid_options = {
            ...
            "body_type": ["Slim", "Athletic", "Average", "Curvy", "Stocky"],
        ...
        for key, options in valid_options.items():
            ...
            if key == "body_type":
                # Normalize common capitalization and spacing for body type values
                val = " ".join([part.capitalize() for part in val.split()])
            if val in options:
                sanitized[key] = val
            else:
